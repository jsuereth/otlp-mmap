...
use crate::{AsyncEventQueue, AttributeLookup, Error};
use otlp_mmap_protocol::{span_event::Event, SpanEvent};
    ...
    fn try_from_event(e: &SpanEvent) -> Result<FullSpanId, Error> {
        Ok(FullSpanId {
            ...
            span_id: e
                ...
                .as_slice()
                .try_into()
                .map_err(|_| Error::InvalidSpanIdError)?,
        })
    }
}
...
impl ActiveSpans {
    ...
    pub async fn try_buffer_spans<Q: AsyncEventQueue<SpanEvent>, L: AttributeLookup>(
        &mut self,
        event_queue: &Q,
        lookup: &L,
        len: usize,
        ...
        loop {
            ...
            tokio::select! {
                event = event_queue.try_read_next() => {
                    ...
                    if let Some(span) = self.try_handle_span_event(event?, lookup)? {
    ...
    fn try_handle_span_event<AL: AttributeLookup>(
        &mut self,
        e: SpanEvent,
        attr_lookup: &AL,
                ...
                if let Some(entry) = self.spans.get_mut(&hash) {
                    for kvr in ae.attributes {
                        entry
                            .current
                            .attributes
                            .push(attr_lookup.try_convert_attribute(kvr)?);
                    }
                }
            }
            Some(Event::End(se)) => {
